...
void executor::on_pool_have_job(size_t pool_id, pool_job& oPoolJob)
{
	...
	if(dat.pool_id != pool_id)
	{
		...
		printer::inst()->print_msg(L3, "New block detected, height = %lu", bswap_64(oPoolJob.iBlockHeight));
}
...
void executor::on_miner_result(size_t pool_id, job_result& oResult)
{
	...
	if(pool->is_dev_pool())
	{
		...
		if(pool->is_running() && pool->is_logged_in())
			pool->cmd_submit(oResult.sJobID, oResult.iNonce, oResult.iNonceUpper, oResult.bResult, backend_name,
	...
	bool bResult = pool->cmd_submit(oResult.sJobID, oResult.iNonce, oResult.iNonceUpper, oResult.bResult,
