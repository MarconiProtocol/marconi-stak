...
struct job_result
{
	...
	char		sJobID[64];
	uint32_t	iNonce;
	uint32_t	iNonceUpper;
	uint32_t	iThreadId;
	xmrstak_algo algorithm = {invalid_algo};

	job_result() {}
	job_result(const char* sJobID, uint32_t iNonce, uint32_t iNonceUpper, const uint8_t* bResult, uint32_t iThreadId, const xmrstak_algo& algo) :
		iNonce(iNonce), iNonceUpper(iNonceUpper), iThreadId(iThreadId), algorithm(algo)
	{
